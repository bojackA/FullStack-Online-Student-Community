...
else
{
    if($movefile)
    {
       ...
       $sql = "INSERT IGNORE INTO `os1`(`name`, `size`, `downloads`)
...
if (isset($_GET['file_id'])) {
    ...
    $sql = "SELECT * FROM os1 WHERE id=$id";
    $result = mysqli_query($conn, $sql);
    ...
    if (file_exists($filepath)) {
        ...
        $updateQuery = "UPDATE os1 SET downloads=$newCount WHERE id=$id";
        mysqli_query($conn, $updateQuery);
        $updateQuery = "UPDATE os1 SET idf=1 WHERE id=$id";
        mysqli_query($conn, $updateQuery);
        exit;
    }
